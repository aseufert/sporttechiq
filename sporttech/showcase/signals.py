...
from showcase.models import PlayerScorecard, Team, Club, Player
...
def callSVGGenerator(sender, signal, instance, **kwargs):
    ...
    total_dribbling = PlayerScorecard.objects.filter(player=instance.player).aggregate(Avg('total_dribbling'))['total_dribbling__avg']
    total_passing   = PlayerScorecard.objects.filter(player=instance.player).aggregate(Avg('total_passing'))['total_passing__avg']
    total_shooting  = PlayerScorecard.objects.filter(player=instance.player).aggregate(Avg('total_shooting'))['total_shooting__avg']
    grand_total     = PlayerScorecard.objects.filter(player=instance.player).aggregate(Avg('grand_total'))['grand_total__avg']
    showcases       = PlayerScorecard.objects.filter(player=instance.player).all()[:5]
    grass_png       = '{}/static/img/grass_tile.png'.format(settings.SERVER_BASE)
    showcase_no     = 0
    scorecard_info  = []
    if instance.player.team:
        if instance.player.team.club.photo:
            club_photo = '{}/media/{}'.format(settings.SERVER_BASE, instance.player.team.club.photo)
        else:
            club_photo = '{}/media/club_logos/club00001.png'.format(settings.SERVER_BASE)
    else:
        club_photo = '{}/media/club_logos/club00001.png'.format(settings.SERVER_BASE)
    ...
    for showcase in showcases:
        ...
        scorecard_info.append(scorecard_data)

    data = {
        'first_name': instance.player.first_name,
        'last_name': instance.player.last_name[0],
        'birth_year': instance.player.birth_year,
        'height': instance.height,
        'country': instance.player.country,
        'city': instance.player.city,
        'state': instance.player.state,
        'club_icon': club_photo,
        'photo': player_photo,
        'no_scorecards': showcase_no,
        'scorecard': scorecard_info,
        'grass_tile': grass_png,
        'avg': {
          'shooting': int(total_shooting),
          'passing': int(total_passing),
          'dribbling': int(total_dribbling),
          'control': int(total_control),
          'skill': int(grand_total)
        }
    }

    svg_file = svggen.svgGen(data)
    Not registering new files until solution can be found
    Player.objects.filter(id=instance.player.id).update(trading_card=svg_file)
    '''
